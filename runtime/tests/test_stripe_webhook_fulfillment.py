...
def gateway(tmp_path, monkeypatch):
    ...
    cfg["access"].update({
        ...
        "checkout": {
            ...
            "fulfillment": "AUTO_WEBHOOK_PLUS_REDIRECT",
            "entitlement_verification": "STRIPE_VERIFIED",
        },
    })
...
def test_webhook_first_then_redirect_still_gets_exactly_one_browser_handoff(gateway):
    ...
    duplicate_redirect = payment_browser.get(f"/checkout/complete/{SLUG}?session_id={SESSION_ID}", follow_redirects=False)
    assert duplicate_redirect.status_code == 409
...
def test_webhook_replay_cannot_resurrect_revoked_payment(gateway):
    module = gateway
    client = TestClient(module.app)
    first = post_event(client, "evt_BeforeRevoke123")
    ...
    assert replay_new_event.status_code in {200, 403}
    assert not module.entitlements.authorize_payment(package_id=SLUG, payment_ref=PAYMENT_REF)


def test_browser_handoff_remains_one_time_after_webhook_fulfillment(gateway):
    module = gateway
    client = TestClient(module.app)
    ...
    assert client.get(activate, follow_redirects=False).status_code == 303
    assert client.get(activate, follow_redirects=False).status_code == 409
