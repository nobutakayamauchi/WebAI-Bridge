...
def retrieve_checkout_session(*, secret_key: str, session_id: str, timeout: float = 10.0) -> dict:
    ...
    try:
        ...
        raise StripeCheckoutError(f"Stripe Checkout Session lookup failed: HTTP {exc.code}") from None
    except URLError as exc:
        raise StripeCheckoutError(f"Stripe Checkout Session lookup failed: {exc.reason}") from None
    except (UnicodeDecodeError, json.JSONDecodeError):
        raise StripeCheckoutError("Stripe Checkout Session response was not valid JSON") from None
    if not isinstance(payload, dict):
        raise StripeCheckoutError("Stripe Checkout Session response was not an object")
    return payload
...
def validate_paid_checkout_session(*, session: dict, app_config: dict) -> dict:
    ...
    if not session.get("payment_link"):
        raise StripeCheckoutError("Stripe Checkout Session is not bound to a Payment Link")
